...
const double kFx = 614.367737; //583 474.874878
const double kFy = 614.367737; //583 474.874878
const double kCx = 308; //320 314.048645
const double kCy = 234.326263; //240 245.994995
...
public:
	...
	//bool readIntrinsicParameterFile(string filename);
        ...
        bool readColorImage(cv::Mat color_img_); //(string filename);

        bool readDepthImage(cv::Mat depth_img); //(string filename);

	bool runPlaneDetection();

	void prepareForMRF();

        cv::Mat writeOutputFiles(bool run_mrf = false);
